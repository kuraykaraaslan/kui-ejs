...
| second tone | `#ec4899` | **the EJS kit** — the runtime this kit targets — the server-rendered sibling of kui-react. Unlike the other packages this pair has no single domain highlight to name, so the second tone names the runtime instead; two kits with one mark would be indistinguishable in a tab strip |
