...
<div class="flex h-screen overflow-hidden bg-surface-base<%= locals.className ? ' '+locals.className : '' %>">
  ...
  <aside class="relative hidden lg:flex flex-col h-full min-h-0 shrink-0 border-r border-border bg-surface-raised">
    ...
    <div class="absolute inset-x-0 top-0 z-10 flex items-center h-14 border-b border-border bg-surface-raised overflow-hidden px-4">
      <%- locals.logoContent %>
    </div>
    <% } %>
    <div class="flex min-h-0 flex-1<%= locals.logoContent ? ' pt-14' : '' %>">
  ...
  <div class="flex flex-1 flex-col min-w-0 min-h-0">
    <% if (_hasTopbar) { %>
    <header class="sticky top-0 z-[40] flex items-center h-14 px-4 border-b border-border bg-surface-raised/90 backdrop-blur shrink-0">
      <% if (_hasSidebar) { %>
      <button type="button" onclick="openDrawer('<%= _drawerId %>')"
        aria-label="Open sidebar"
        class="inline-flex lg:hidden items-center justify-center w-9 h-9 rounded-md text-text-secondary hover:text-text-primary hover:bg-surface-overlay transition-colors mr-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-focus">
  ...
  <% if (_hasSidebar) { %>
  <%- include('../ui/Drawer', { id: _drawerId, title: _title, side: 'left', open: false, children: locals.sidebarContent || '' }) %>
  <% } %>
</div>
