...
<div id="<%= _ssid %>" class="relative<%= locals.className ? ' ' + locals.className : '' %>">
  ...
  >
    ...
    <div class="flex-1 min-w-0 text-left">
      <p class="font-mono text-xs text-text-primary truncate" data-ss-active-url><%= active.url %></p>
      <% if (active.description) { %>
        <p class="text-xs text-text-secondary mt-0.5 line-clamp-1" data-ss-active-desc><%= active.description %></p>
      <% } %>
    </div>
...
<script>(function(){
  ...
  root.querySelectorAll('[data-ss-option]').forEach(function(li){
    li.addEventListener('click', function(){
      ...
      var descEl = root.querySelector('[data-ss-active-desc]');
