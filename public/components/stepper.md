...
<%
  ...
  var stateStyles = {
    ...
    error:    { circle: 'bg-error text-text-inverse border-error',             text: 'text-error-fg',               line: 'bg-border' },
  ...
  <li class="flex gap-3 items-start">
    <div class="flex flex-col items-center shrink-0">
      <div class="h-8 w-8 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0 <%= s.circle %>"
        ...
        <% if (state === 'complete') { %><span class="w-3.5 h-3.5 inline-flex items-center justify-center"><i class="fa-solid fa-check" aria-hidden="true"></i></span>
        <% } else if (state === 'error') { %><span class="w-3.5 h-3.5 inline-flex items-center justify-center"><i class="fa-solid fa-xmark" aria-hidden="true"></i></span>
        <% } else { %><%= i + 1 %><% } %>
      </div>
      <% if (!isLast) { %><div class="w-0.5 flex-1 min-h-[2rem] mt-1 <%= s.line %>" aria-hidden="true"></div><% } %>
    </div>
...
<ol class="flex items-center<%= _className ? ' ' + _className : '' %>">
  ...
  <li class="flex items-center<%= !isLast ? ' flex-1' : '' %>">
    ...
    <% if (!isLast) { %><div class="h-0.5 flex-1 mx-2 mt-[-1.25rem] <%= s.line %>" aria-hidden="true"></div><% } %>
  </li>
  <% }); %>
</ol>
<% } %>
