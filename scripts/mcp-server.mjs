#!/usr/bin/env node
// kui-ejs MCP server (stdio, zero-dep).
//
// Exposes the partial registry to MCP-compatible AI clients (Claude
// Desktop, Cursor, Cline, Windsurf, Zed). Implements JSON-RPC 2.0 framed
// over stdio per the Model Context Protocol spec.
//
// Reads the static snapshot at public/registry/components.json. Refresh
// with `npm run registry:snapshot`.
...
const TOOLS = [
  {
    ...
    description: 'List EJS partials in the kui-ejs registry. Optional filter: layer (ui|app|domain|theme) or category.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'get_component',
    description: 'Get a single partial by id, with full EJS source, variants, composes[] (include order), a11y metadata, and design tokens.',
    inputSchema: {
      type: 'object',
      required: ['id'],
  ...
  },
  {
    ...
    description: 'Substring search across partial name + description + filePath. Returns up to `limit` matches (default 20).',
    inputSchema: {
      type: 'object',
      required: ['query'],
      properties: {
  ...
  },
  {
    ...
    description: 'Return the EJS authoring conventions (templating, icons, styling, includes, accessibility, raw output, file naming).',
    inputSchema: { type: 'object', properties: {} },
  },
  {
  ...
  {
    name: 'read_file',
    description: 'Read a file from disk. Path must be repo-relative (e.g. "modules/ui/Button.ejs") and exist within the repository root.',
    inputSchema: {
      type: 'object',
      required: ['path'],
...
async function callTool(name, args = {}) {
  ...
  switch (name) {
    ...
    case 'list_themes':       return jsonResult(reg.themes);
    case 'get_conventions':   return jsonResult(reg.conventions);
    case 'list_design_tokens':return jsonResult(reg.designTokens);
    case 'read_file': {
      const rel = String(args.path || '');
      const abs = path.resolve(REPO_ROOT, rel);
      if (!abs.startsWith(REPO_ROOT + path.sep)) return errorResult(`Refusing path outside repo: ${rel}`);
      try {
...
const SERVER_INFO = { name: 'kui-ejs-registry', version: '1.0.0' };
